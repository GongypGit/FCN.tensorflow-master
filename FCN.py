...
"""
vgg_net:
kernel.shape:[width, height, in_channels, out_channels]
conv1_1
...
def inference(image, keep_prob):
    ...
    with tf.variable_scope("inference"):
        ...
        ''' 
        deconv_shape1:(?, 14, 14, 512)
        '''
        W_t1 = utils.weight_variable([4, 4, deconv_shape1[3].value, NUM_OF_CLASSESS], name="W_t1")
        '''具有[filter_height, filter_width, out_channels, in_channels]
...
def main(argv=None):
    with tf.Session() as sess:
        with tf.device("/cpu:0"):
            ...
            # print(np.shape(pred_annotation))
            tf.summary.image("input_image", image, max_outputs=2)
            tf.summary.image("ground_truth", tf.cast(annotation, tf.uint8), max_outputs=2)
            '''
            tf.cast(x, dtype, name=None):convert the data format of x to dtype
            ...
            if FLAGS.mode == "train":
                for itr in range(MAX_ITERATION):
                    ...
                    feed_dict = {image: train_images, annotation: train_annotations, keep_probability: FLAGS.pro_b}
        
                    sess.run(train_op, feed_dict=feed_dict)
                    if itr <100 :
                        FLAGS.learning_rate = 1e-4 - 5e-7 * itr
                    elif itr <500:
                        FLAGS.learning_rate = 6e-5 - 1e-7 * itr
                    else:
                        FLAGS.learning_rate = 13e-6 - 6e-9 * itr
